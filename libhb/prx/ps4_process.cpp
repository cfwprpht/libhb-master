...
int LibHomebrew::Loot::Proc::UserLoadExec(const char *path, void *arg1, void *arg2) {
	...
	if (!elf) { _printf("[PS4-Process] Couldn't load elf from path : %s\n", path); return -1; }
	fread(buf, 1, sizeof(buf), elf);
	ptr = reinterpret_cast<int (*)(void *, void *)>(ElfLoader::LoadImage(buf, sizeof(buf)));
	if (!ptr) return -1;

	_printf("[PS4-Process] Executing Mapped ELF now.\n");
...
int LibHomebrew::Loot::Proc::LoadExecBin(const char *path, void *arg1, void *arg2) {
	...
	if (!fd) { _printf("[PS4-Process] Couldn't load bin from path : %s\n", path); return -1; }
	...
	if (!execute) { _printf("[PS4-Process] Can't allocate memory\n"); return -1; }
	...
	_printf("[PS4-Process] Executing Mapped Binary now.\n");
	...
	if (file.Exists()) {
		if (file.Open(IO::Read, IO::BYTE)) {
			...
			if (!file.Read(buffer, info.Length())) {
				...
				_printf("[PS4-Process] Kexec file load error.\n");
				return 1;
			}
			file.Close();
		} else return 1;
