...
void LibHomebrew::Loot::ElfLoader::Resolve(const char *sym, void *offset) {
	...
	if (handleLibc == 0) {		
		...
		if (!handleLibc) {
			_printf("[ELF-Loader] Couldn't load module : libc.sprx.\n");
			_printf("[ELF-Loader] %s\n", strerror(errno));
		}
	}
		...
		if (handleLibkernel == 0) {
			...
			if (!handleLibkernel) {
				...
				if (!handleLibkernel) {
					...
					if (!handleLibkernel) {
						_printf("[ELF-Loader] Couldn't load module : libkernel.sprx.\n");
						_printf("[ELF-Loader] %s\n", strerror(errno));
					}
				}
			}
	...
	if (ret != SCE_OK) {
		...
		if (handleLibFios2 == 0) {
			...
			if (!handleLibFios2) {
				_printf("[ELF-Loader] Couldn't load module : libSceFios2.sprx.\n");
				_printf("[ELF-Loader] %s\n", strerror(errno));
			}
		}
	} else return;
	...
	if (ret != SCE_OK) {
		...
		if (handleLibSystemService == 0) {
			...
			if (!handleLibSystemService) {
				_printf("[ELF-Loader] Couldn't load module : libSceSystemService.sprx.\n");
				_printf("[ELF-Loader] %s\n", strerror(errno));
			}
		}
	} else return;
	ret = sceKernelDlsym(handleLibSystemService, sym, &offset);
	if (ret != SCE_OK) _printf("[ELF-Loader] %s\n", strerror(errno));
}
	...
	for (j = 0; j < shdr->sh_size / sizeof(Elf32_Rel); j += 1) {
		...
		switch (ELF64_R_TYPE(rel[j].r_info)) {
			...
			case R_386_GLOB_DAT:
				Elf64_Word addr;
				Resolve(sym, &addr);
				if (!addr) _printf("[ELF-Loader]  Couldn't relocate %s !\n", sym);
...
void *LibHomebrew::Loot::ElfLoader::LoadImage(char *elf_start, unsigned int size) {
	...
	_printf("[ELF-Loader] New EHDR set.\n");

	_printf("[ELF-Loader] Checking Header...");
	if (!isIdentOk(hdr)) { _printf("Magic missmatch : Not a ELF File !\n"); return 0; }
	_printf("ok !\n");

	_printf("[ELF-Loader] Mapping Memory...");
	exec = (char *)Sys::mmap(NULL, size, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_PRIVATE | MAP_ANONYMOUS, 0, 0);
	if (!exec) { _printf("error : Allocating memory !\n"); return 0; }
	_printf("ok !\n");	

	// Start with clean memory.
	memset(exec, 0x0, size);
	phdr = (Elf64_Phdr *)(elf_start + hdr->e_phoff);
	_printf("[ELF-Loader] Memory cleaned.\n");

	_printf("[ELF-Loader] Mapping Programms...");
	for (i = 0; i < hdr->e_phnum; ++i) {
		if (phdr[i].p_type != PT_LOAD) continue;
		if (phdr[i].p_filesz > phdr[i].p_memsz) {
			_printf("error : p_filesz > p_memsz\n");
	...
	_printf("[ELF-Loader] New SHDR set.\n");

	_printf("[ELF-Loader] Remapping Tables and Entry point...");
	...
	_printf("[ELF-Loader] Try to relocate as much as possible.\n");
	for (i = 0; i < hdr->e_shnum; ++i) {
		if (shdr[i].sh_type == SHT_REL) Relocate(shdr + i, syms, strings, elf_start, exec);
	}
	_printf("[ELF-Loader] ELF Loaded and Armed.\n");
