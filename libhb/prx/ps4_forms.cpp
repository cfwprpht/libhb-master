...
void PS4Forms::PictureBox::Draw(ssg::GraphicsContext *context, ssg::SpriteRenderer *sprite) {
	if (show & visible) {
		// Check first.
		if (_texture == NULL) {
			_printf("[PictureBox] texture is null : %s(%p).\n", _id, this);
			return;
		}
		if ((_texture->getWidth() == 0) || (_texture->getHeight() == 0)) {
			_printf("[PictureBox] texture size is zero.\n");
			return;
		}
		if ((_size.getX() == 0) || (_size.getY() == 0)) {
			_printf("[PictureBox] Either x and/or y coordinate are zero.\n");
			return;
		}
...
int PS4Forms::TextViewer::LoadFile(const char *path) {
	if (path == nullptr) {
		_printf("[TextViewer] No Path overloaded.\n");
		return -1;
	} else if (path[0] == '\0') {
		_printf("[TextViewer] Path of file to load is empty.\n");
		return -2;
	}
	...
	if (fd) {
		...
		return 0;
	} else _printf("[TextViewer] Couldn't open file for reading.\n");
	return -3;
}
...
int PS4Forms::HexViewer::LoadFile(const char *path) {
	if (path == nullptr) {
		_printf("[HexViewer] No Path overloaded.\n");
		return -1;
	}
	else if (path[0] == '\0') {
		_printf("[HexViewer] Path of file to load is empty.\n");
		return -2;
	}
	...
	else _printf("[HexViewer] Couldn't open file for reading.\n");
	return -3;
}
