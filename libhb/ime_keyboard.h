...
public:
	int initialize() {
		...
		if (loadModuleResult != SCE_OK) {
			printf("Error loading IME PRX: 0x%8X\n", loadModuleResult);
		}
	...
	int finalize() {
		...
		if (unloadModuleResult != SCE_OK) {
			printf("Error unloading IME PRX: 0x%8X\n", unloadModuleResult);
		}
	...
	int start_keyboard(const SceUserServiceUserId userId, uint32_t *state) {
		...
		if ((ret = sceImeKeyboardOpen(userId, &kparam)) != SCE_OK) {
			if (ret == SCE_IME_ERROR_NOT_ACTIVE) {
				...
				printf("    # keyboard : sceImeKeyboardOpen() error:0x%08x #\n", ret);
			}
			return ret;
		}
		printf("    # keyboard : sceImeKeyboardOpen() Success #\n");
			...
			for (int idx = 0; idx < SCE_IME_KEYBOARD_MAX_NUMBER; ++idx) {
				if (resourceIdArray.resourceId[idx] != SCE_IME_KEYBOARD_RESOURCE_ID_INVALID) {
					...
					if ((ret = sceImeKeyboardGetInfo(resourceIdArray.resourceId[idx], &info)) != SCE_OK) {
						printf("    # keyboard : sceImeKeyboardGetInfo() error:0x%08x #\n", ret);
					}
					else {
						printf("    # keyboard : sceImeKeyboardGetInfo() Success:[idx=%d] uid=%08x type=%d#\n", idx, info.userId, info.type);
					}
				}
			}
		}
	...
	void end_keyboard(const SceUserServiceUserId userId) {
		...
		if (ret != SCE_OK) {
			printf("    # keyboard : sceImeKeyboardClose() Error: userId=%08x ret=%08x#\n", userId, ret);
			return;
		}
		printf("    # keyboard : sceImeKeyboardClose() Success: userId=%08x#\n", userId);
	}
};