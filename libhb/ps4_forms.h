...
namespace PS4Forms {
	...
	class PictureBox : public WidgetBase {
	public:
		enum HAlign {
			kHAlignLeft,
			kHAlignRight,
	...
	public:
		...
		// Adds text to the buffer
		static void onScreenPrintf(bool isDir, const char *text, ...);

		// Adds text to the buffer
		static void onScreenPrintf(bool isDir, PictureBox *icon, const char *text, ...);

		// Adds text to the buffer with a certain colour
		static void onScreenPrintf(const uint32_t &colour, const char *text, ...);

		// Adds text to the buffer with a certain colour
		static void onScreenPrintf(const uint32_t &colour, bool isDir, const char *text, ...);

		// Adds text to the buffer with a certain colour
		static void onScreenPrintf(const uint32_t &colour, bool isDir, PictureBox *icon, const char *text, ...);
		...
		// Set a EventDispatcher to use for sound effects.
		static void setEvent(common::Service::EventDispatcher *eventDispatcher);
	...
	public:
		...
		void setEvent(common::Service::EventDispatcher *eventDispatcher);
		void deleteLastLine(void);
		bool isSelectedLineDir(void);
	...
	class RichTextBox : public WidgetBase {
	protected:		
		TTYD                      *ttyd;
...
#pragma clang diagnostic pop
	...
	class HexViewer : public TextViewer {
	protected:
		byte                  *buffer;
